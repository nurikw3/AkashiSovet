...
SQL-файлы в каталоге `migrations/` применяйте к своей БД вручную или через свой пайплайн (отдельного job в compose нет). После обновления с вводом таймзоны Астаны примените в том числе `005_meetings_scheduled_timestamptz.sql`, если ещё не накатывали.
