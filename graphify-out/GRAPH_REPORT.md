...
3. `get_app_pdf_buffer()` - 30 edges
4. `generate_pdf()` - 26 edges
5. `invalidate_pdf_cache()` - 25 edges
6. `meetings_create()` - 22 edges
7. `_parse_meeting_schedule()` - 20 edges
8. `meeting_delete()` - 20 edges
9. `ApplicationTemplate` - 20 edges
10. `_meeting_form_err_prefix()` - 18 edges
...
Cohesion: 0.05
Nodes (38): _blocks_preview_html(), broadcast_superusers_html(), notify_user_application_rework(), Отправка уведомлений пользователям через Telegram Bot API., Уведомляет автора о возврате на доработку., Короткий HTML-превью текста заявки для уведомления о доработке., Короткий HTML-превью текста заявки для уведомления о доработке., Рассылает HTML всем суперпользователям (например при ошибке генерации PDF). (+30 more)

### Community 4 - "Community 4"
Cohesion: 0.07
Nodes (36): _block_label(), _build_messages(), format_text(), _make_cache_key(), Форматирование и генерация текста блоков через LLM., Изоляция кэша по заявке и блоку — одинаковый «test» не бьёт в чужой контекст., В ключ входят заявка (и user), иначе Redis отдаёт ответ от другой заявки с тем ж, В ключ входят заявка (и user), иначе Redis отдаёт ответ от другой заявки с тем ж (+28 more)
...
Cohesion: 0.09
Nodes (36): clear_chat_history(), delete_app(), get_app(), get_chat_history(), get_draft_id_for_user(), get_last_rework_app(), get_or_create_app(), get_pending_apps() (+28 more)
...
Cohesion: 0.11
Nodes (19): _cache_key(), get_app_pdf_buffer(), _get_pdf_template_revision(), _get_redis(), _pdf_cache_token(), Универсальная функция подготовки и генерации PDF для заявки.     Единый источник, Отдаёт PDF из S3 (если есть флаг в Redis) или генерирует заново.     Redis храни, Универсальная функция подготовки и генерации PDF для заявки.     Единый источник (+11 more)
...
Cohesion: 0.12
Nodes (18): delete_meeting_by_id(), get_meeting_by_id(), Создаёт заседание; возвращает строку как dict (RETURNING *)., Одна запись `meetings` по id., Одна запись `meetings` по id., Одна запись `meetings` по id., Одна запись `meetings` по id., Одна запись `meetings` по id. (+10 more)
...
Cohesion: 0.2
Nodes (14): cb_back(), cb_del(), cb_del_ok(), cb_page(), cb_view(), cmd_my_apps(), _format_card(), _get_apps_page() (+6 more)
...
Cohesion: 0.13
Nodes (16): extend_meeting_application_ids(), list_meetings_past(), _parse_application_ids_jsonb(), Прошедшие заседания, от новых к старым., Прошедшие заседания, от новых к старым., Прошедшие заседания, от новых к старым., Прошедшие заседания, от новых к старым., Прошедшие заседания, от новых к старым. (+8 more)
...
Cohesion: 0.13
Nodes (15): get_application_status_by_ids(), list_meetings_upcoming(), id заявки → status (только для существующих id из списка)., id заявки → status (только для существующих id из списка)., id заявки → status (только для существующих id из списка)., id заявки → status (только для существующих id из списка)., id заявки → status (только для существующих id из списка)., id заявки → status (только для существующих id из списка). (+7 more)
...
Cohesion: 0.16
Nodes (12): Flowable, _append_section_paragraphs(), _generate_pdf_sync(), _LogoPageMarker, _normalize_pdf_user_text(), _normalize_risk_placeholder(), _parse_attachments_field(), Невидимый маркер: страница считается «логотипной», если есть старт секции. (+4 more)

### Community 29 - "Community 29"
Cohesion: 0.14
Nodes (13): main(), generate_pdf(), _load_signature_for_user(), Генерирует PDF служебной записки с авто-подписью., Генерирует PDF без кэша. Используй get_app_pdf_buffer для кэшированной версии., Генерирует PDF без кэша. Используй get_app_pdf_buffer для кэшированной версии., Генерирует PDF без кэша. Используй get_app_pdf_buffer для кэшированной версии., Генерирует PDF без кэша. Используй get_app_pdf_buffer для кэшированной версии. (+5 more)

### Community 30 - "Community 30"
Cohesion: 0.15
Nodes (14): _bump_pdf_template_revision(), invalidate_template_cache(), persist_template(), Удаляет кэш шаблона в Redis (вызывать после обновления записи в БД)., Удаляет кэш шаблона в Redis (вызывать после обновления записи в БД)., Сохраняет шаблон в `settings` и сбрасывает Redis-кэш., Сохраняет шаблон в `settings` и сбрасывает Redis-кэш., Удаляет кэш шаблона в Redis (вызывать после обновления записи в БД). (+6 more)
...
Cohesion: 0.14
Nodes (14): invalidate_application_pdf_cache(), invalidate_pdf_cache(), Инвалидирует кэш PDF для заявки после изменения данных.      Сейчас `get_app_pdf, Инвалидирует кэш PDF для заявки после изменения данных.      Сейчас `get_app_pdf, Инвалидирует кэш PDF для заявки после изменения данных.      Сейчас `get_app_pdf, Инвалидирует кэш PDF для заявки после изменения данных.      Сейчас `get_app_pdf, Сбрасывает Redis-токен и удаляет объект PDF в S3 для заявки., Сбрасывает Redis-токен и удаляет объект PDF в S3 для заявки. (+6 more)
...
Cohesion: 0.15
Nodes (13): draw_background(), Логотип на всех страницах кроме последней., Логотип на всех страницах кроме последней., Логотип на всех страницах кроме последней., Логотип на всех страницах кроме последней., Для блока про риски сохраняем отображение «не применимо» как в старой вёрстке., Обычный Canvas, подпись теперь рисуется в самом тексте., Для блока про риски сохраняем отображение «не применимо» как в старой вёрстке. (+5 more)
...
Cohesion: 0.17
Nodes (6): get_application_record(), Операции с заявкой: создание черновика, смена статуса, отправка на согласование,, Сырая строка из БД (как `db.get_app`): нужна, где важен исходный JSON вложений (, Сырая строка из БД (как `db.get_app`): нужна, где важен исходный JSON вложений (, save_all_blocks(), save_block()
...
Cohesion: 0.2
Nodes (8): Canvas, _LastPageCanvas, Обычный Canvas, подпись теперь рисуется в самом тексте., Обычный Canvas, подпись теперь рисуется в самом тексте., Обычный Canvas, подпись теперь рисуется в самом тексте., Обычный Canvas, подпись теперь рисуется в самом тексте., Обычный Canvas, подпись теперь рисуется в самом тексте., Обычный Canvas, подпись теперь рисуется в самом тексте.
...
Cohesion: 0.22
Nodes (9): _load_signature_for_ref(), Берёт подпись пользователя: из S3 по ключу в БД либо устаревшие бинарные данные., Берёт подпись пользователя: из S3 по ключу в БД либо устаревшие бинарные данные., Берёт подпись пользователя: из S3 по ключу в БД либо устаревшие бинарные данные., Берёт подпись пользователя: из S3 по ключу в БД либо устаревшие бинарные данные., Берёт подпись пользователя: из S3 по ключу в БД либо устаревшие бинарные данные., Берёт подпись пользователя: из S3 по ключу в БД либо устаревшие бинарные данные., Загружает подпись по уже известному ref без дополнительного запроса в БД. (+1 more)
...
Cohesion: 0.25
Nodes (8): draw_last_page(), Логотип + футер — только на последней странице., Логотип + футер — только на последней странице., Логотип + футер — только на последней странице., Логотип + футер — только на последней странице., Логотип + футер — только на последней странице., Логотип + футер — только на последней странице., Рисует оформление последней страницы (опциональный логотип + футер).

### Community 51 - "Community 51"
Cohesion: 0.29
Nodes (6): get_cached_llm_response(), save_llm_response_to_cache(), get_redis(), Клиент Redis из `redis_client` (может быть None, если URL не задан)., Клиент Redis из `redis_client` (может быть None, если URL не задан)., dir()
...
## Knowledge Gaps
- **529 isolated node(s):** `Подписанная cookie админ-сессии (HMAC). Без секрета — только для разработки.`, `Разбирает cookie; при WEB_SESSION_SECRET отвергает подделку.`, `Только удаление строки в форме (без записи в БД).      HTMX по умолчанию не дела`, `Фрагмент HTMX: виджеты счётчиков (обновление без перезагрузки).`, `Возвращает уже согласованную заявку на доработку (только суперпользователь).` (+524 more)
...
- **Why does `get_template()` connect `Community 31` to `Community 1`, `Community 3`, `Community 4`, `Community 41`, `Community 11`, `Community 44`, `Community 13`, `Community 17`, `Community 50`, `Community 51`, `Community 52`, `Community 21`, `Community 29`?**
  _High betweenness centrality (0.184) - this node is a cross-community bridge._
- **Why does `invalidate_pdf_cache()` connect `Community 33` to `Community 65`, `Community 3`, `Community 68`, `Community 38`, `Community 11`, `Community 13`, `Community 14`, `Community 55`, `Community 60`?**
  _High betweenness centrality (0.084) - this node is a cross-community bridge._
- **Why does `generate_pdf()` connect `Community 29` to `Community 33`, `Community 2`, `Community 36`, `Community 13`, `Community 46`, `Community 14`, `Community 49`, `Community 28`, `Community 31`?**
