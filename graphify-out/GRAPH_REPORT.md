...
Cohesion: 0.15
Nodes (20): BaseModel, Application, ApplicationAttachment, Meeting, Пользователь Telegram / запись в таблице `users`., Пользователь Telegram / запись в таблице `users`., Заседание Правления: планируемая сущность из `meetings` (id — после появления та, Заседание Правления: планируемая сущность из `meetings` (id — после появления та (+12 more)
...
Cohesion: 0.15
Nodes (10): application_detail_page(), _clean_attachments(), dashboard(), _get_hashed_password(), login(), meeting_detail_page(), _parse_app(), settings_page() (+2 more)
...
Cohesion: 0.17
Nodes (11): clean(), Очистка markdown в значениях перед model_validate., Очистка markdown и склеенных нумерованных списков перед model_validate., _strip_md(), strip_submit_memo_args(), strip_text(), SubmitMemo, expand_numbered_newlines() (+3 more)

### Community 21 - "Community 21"
Cohesion: 0.12
Nodes (16): Сохраняет список вложений как в БД (в т.ч. реестр файлов из Telegram до выгрузки, save_attachments_payload(), save_block(), _cache_key(), _get_redis(), invalidate_pdf_cache(), Инвалидирует кэш PDF для заявки после изменения данных.      Сейчас `get_app_pdf, Инвалидирует кэш PDF для заявки после изменения данных.      Сейчас `get_app_pdf (+8 more)

### Community 22 - "Community 22"
Cohesion: 0.13
Nodes (9): clear_application_chat_history(), get_application_record(), get_draft_application_id_for_user(), invalidate_application_pdf_cache(), Операции с заявкой: создание черновика, смена статуса, отправка на согласование,, Сырая строка из БД (как `db.get_app`): нужна, где важен исходный JSON вложений (, Обнуляет историю free-form / LLM по заявке., `id` черновика пользователя, если есть. (+1 more)
...
Cohesion: 0.15
Nodes (8): ApplicationTemplate, ChatMessage, Централизованные Pydantic-модели домена (заявки, пользователи, шаблон, заседания, Динамический шаблон структуры заявки (блоки — см. `stdlib.template.ApplicationTe, Динамический шаблон структуры заявки (блоки — см. `stdlib.template.ApplicationTe, Сообщение в истории чата с LLM по заявке., Сообщение в истории чата с LLM по заявке., Template
...
Cohesion: 0.18
Nodes (8): broadcast_superusers_html(), Рассылает HTML всем суперпользователям (например при ошибке генерации PDF)., now_app(), Текущие дата и время в часовом поясе приложения., finalize_and_notify(), on_review_edit(), on_review_submit(), on_rework_submit()
...
- **Why does `get_template()` connect `Community 30` to `Community 1`, `Community 34`, `Community 35`, `Community 37`, `Community 5`, `Community 8`, `Community 10`, `Community 11`, `Community 13`, `Community 15`, `Community 17`, `Community 19`?**
  _High betweenness centrality (0.212) - this node is a cross-community bridge._
- **Why does `generate_pdf()` connect `Community 13` to `Community 0`, `Community 19`, `Community 24`, `Community 29`, `Community 30`?**
  _High betweenness centrality (0.088) - this node is a cross-community bridge._
- **Why does `invalidate_pdf_cache()` connect `Community 21` to `Community 35`, `Community 36`, `Community 37`, `Community 71`, `Community 13`, `Community 22`, `Community 24`?**
