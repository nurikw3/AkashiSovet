...
Cohesion: 0.18
Nodes (17): BaseModel, ApplicationAttachment, ChatMessage, Пользователь Telegram / запись в таблице `users`., Пользователь Telegram / запись в таблице `users`., Элемент списка вложений: S3 (после выгрузки) или Telegram file_id (как в боте)., Сообщение в истории чата с LLM по заявке., Сообщение в истории чата с LLM по заявке. (+9 more)

### Community 17 - "Community 17"
Cohesion: 0.17
Nodes (17): _append_section_paragraphs(), _build_pdf_sync(), _cache_key(), _generate_pdf_sync(), _get_pdf_template_revision(), _get_redis(), _normalize_pdf_user_text(), _normalize_risk_placeholder() (+9 more)
...
Cohesion: 0.14
Nodes (8): get_application_record(), get_draft_application_id_for_user(), Операции с заявкой: создание черновика, смена статуса, отправка на согласование,, Сырая строка из БД (как `db.get_app`): нужна, где важен исходный JSON вложений (, Сырая строка из БД (как `db.get_app`): нужна, где важен исходный JSON вложений (, `id` черновика пользователя, если есть., `id` черновика пользователя, если есть., save_all_blocks()

### Community 31 - "Community 31"
Cohesion: 0.14
Nodes (14): invalidate_application_pdf_cache(), save_block(), invalidate_pdf_cache(), Инвалидирует кэш PDF для заявки после изменения данных.      Сейчас `get_app_pdf, Инвалидирует кэш PDF для заявки после изменения данных.      Сейчас `get_app_pdf, Инвалидирует кэш PDF для заявки после изменения данных.      Сейчас `get_app_pdf, Инвалидирует кэш PDF для заявки после изменения данных.      Сейчас `get_app_pdf, Сбрасывает Redis-токен и удаляет объект PDF в S3 для заявки. (+6 more)
...
Cohesion: 0.24
Nodes (9): escape_markdown_v2(), is_delegation(), handle_block_input(), on_confirm(), Текущий шаг cur идёт позже блока bid в шаблоне (юзер уже продвинулся вперёд)., _remember_cleanup_message(), _send_confirm(), _tpl_block_order_after() (+1 more)

### Community 41 - "Community 41"
Cohesion: 0.24
Nodes (7): clean(), Очистка markdown в значениях перед model_validate., Очистка markdown и склеенных нумерованных списков перед model_validate., _strip_md(), strip_submit_memo_args(), strip_text(), SubmitMemo
...
Cohesion: 0.24
Nodes (9): build_files_step_message(), build_review_text_snapshot(), chunk_plain_text(), html_pre_block(), Единое текстовое представление блоков заявки (plain + HTML с <pre> для Telegram), Один экранированный блок <pre> (parse_mode=HTML)., Сообщение после сбора блоков: сводка в monospace + приглашение к вложениям., Краткая текстовая копия заявки перед отправкой (рядом с PDF). (+1 more)
...
Cohesion: 0.25
Nodes (6): format_blocks_plain_copy(), Все блоки подряд: нумерация, заголовок шаблона, текст — удобно копировать., on_files_done(), on_review_edit(), on_review_submit(), send_review_screen()
...
Cohesion: 0.29
Nodes (6): _normalize_submit_memo_args(), process_free_form_chat(), Free-form диалог для сбора данных заявки., build_submit_memo_model(), Модель для OpenAI tool submit_memo: поля совпадают с блоками шаблона., Модель для OpenAI tool submit_memo: поля совпадают с блоками шаблона.
...
Cohesion: 0.67
Nodes (3): Сохраняет список вложений как в БД (в т.ч. реестр файлов из Telegram до выгрузки, Сохраняет список вложений как в БД (в т.ч. реестр файлов из Telegram до выгрузки, save_attachments_payload()
...
## Knowledge Gaps
- **516 isolated node(s):** `Подписанная cookie админ-сессии (HMAC). Без секрета — только для разработки.`, `Разбирает cookie; при WEB_SESSION_SECRET отвергает подделку.`, `Только удаление строки в форме (без записи в БД).      HTMX по умолчанию не дела`, `Фрагмент HTMX: виджеты счётчиков (обновление без перезагрузки).`, `Возвращает уже согласованную заявку на доработку (только суперпользователь).` (+511 more)
...
- **Why does `get_template()` connect `Community 23` to `Community 1`, `Community 3`, `Community 38`, `Community 7`, `Community 40`, `Community 11`, `Community 12`, `Community 46`, `Community 14`, `Community 15`, `Community 53`, `Community 21`, `Community 57`, `Community 29`?**
  _High betweenness centrality (0.208) - this node is a cross-community bridge._
- **Why does `generate_pdf()` connect `Community 14` to `Community 0`, `Community 36`, `Community 17`, `Community 23`, `Community 29`, `Community 31`?**
  _High betweenness centrality (0.074) - this node is a cross-community bridge._
- **Why does `ApplicationTemplate` connect `Community 16` to `Community 1`, `Community 35`, `Community 68`, `Community 69`, `Community 42`, `Community 81`?**
