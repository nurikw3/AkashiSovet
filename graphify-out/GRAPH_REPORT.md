...
Cohesion: 0.11
Nodes (32): delete_app(), get_app(), get_chat_history(), get_draft_id_for_user(), get_last_rework_app(), get_or_create_app(), get_pending_apps(), get_user_full_name() (+24 more)
...
Cohesion: 0.1
Nodes (22): is_delegation(), _get_context(), handle_block_input(), _handle_delegation(), _handle_format(), Обычный флоу — редактируем то что написал юзер., Обычный флоу — редактируем то что написал юзер., Делегирование — генерируем текст блока из контекста. (+14 more)
...
Cohesion: 0.11
Nodes (19): main(), generate_pdf(), _load_signature_for_user(), Генерирует PDF служебной записки с авто-подписью., Берёт подпись пользователя: из S3 по ключу в БД либо устаревшие бинарные данные., Берёт подпись пользователя: из S3 по ключу в БД либо устаревшие бинарные данные., Генерирует PDF служебной записки с авто-подписью.      Заявка из БД: передайте `, Берёт подпись пользователя: из S3 по ключу в БД либо устаревшие бинарные данные. (+11 more)

### Community 15 - "Community 15"
Cohesion: 0.16
Nodes (18): _append_section_paragraphs(), _build_pdf_sync(), _generate_pdf_sync(), _normalize_blocks_payload(), _normalize_pdf_user_text(), _normalize_risk_placeholder(), _parse_attachments_field(), PDF-генерация служебной записки через ReportLab с фоном из шаблона. (+10 more)

### Community 16 - "Community 16"
Cohesion: 0.17
Nodes (17): BaseModel, ApplicationAttachment, ChatMessage, Meeting, Пользователь Telegram / запись в таблице `users`., Пользователь Telegram / запись в таблице `users`., Заседание Правления: планируемая сущность из `meetings` (id — после появления та, Заседание Правления: планируемая сущность из `meetings` (id — после появления та (+9 more)
...
Cohesion: 0.14
Nodes (13): clean(), FormatResult, LLMComplete, Очистка markdown в значениях перед model_validate., Очистка markdown и склеенных нумерованных списков перед model_validate., _strip_md(), strip_submit_memo_args(), strip_text() (+5 more)

### Community 19 - "Community 19"
Cohesion: 0.12
Nodes (17): Сохраняет список вложений как в БД (в т.ч. реестр файлов из Telegram до выгрузки, Сохраняет список вложений как в БД (в т.ч. реестр файлов из Telegram до выгрузки, save_attachments_payload(), save_block(), _cache_key(), invalidate_pdf_cache(), Инвалидирует кэш PDF для заявки после изменения данных.      Сейчас `get_app_pdf, Инвалидирует кэш PDF для заявки после изменения данных.      Сейчас `get_app_pdf (+9 more)
...
Cohesion: 0.19
Nodes (15): escape_markdown_v2(), build_files_step_message(), build_review_text_snapshot(), chunk_plain_text(), format_blocks_plain_copy(), html_pre_block(), Единое текстовое представление блоков заявки (plain + HTML с <pre> для Telegram), Все блоки подряд: нумерация, заголовок шаблона, текст — удобно копировать. (+7 more)
...
Cohesion: 0.14
Nodes (14): _meeting_form_err_prefix(), Куда редиректить ошибку валидации: страница списка заседаний или дашборд approve, Куда редиректить ошибку валидации: страница списка заседаний или дашборд approve, Куда редиректить ошибку валидации: страница списка заседаний или дашборд approve, Куда редиректить ошибку валидации: страница списка заседаний или дашборд approve, Куда редиректить ошибку валидации: страница списка заседаний или дашборд approve, Куда редиректить ошибку валидации: страница списка заседаний или дашборд approve, Куда редиректить ошибку валидации: страница списка заседаний или дашборд approve (+6 more)
...
Cohesion: 0.17
Nodes (12): _blocks_preview_html(), broadcast_superusers_html(), notify_user_application_rework(), Отправка уведомлений пользователям через Telegram Bot API., Уведомляет автора о возврате на доработку., Короткий HTML-превью текста заявки для уведомления о доработке., Короткий HTML-превью текста заявки для уведомления о доработке., Рассылает HTML всем суперпользователям (например при ошибке генерации PDF). (+4 more)
...
Cohesion: 0.15
Nodes (8): ApplicationTemplate, Application, Централизованные Pydantic-модели домена (заявки, пользователи, шаблон, заседания, Динамический шаблон структуры заявки (блоки — см. `stdlib.template.ApplicationTe, Динамический шаблон структуры заявки (блоки — см. `stdlib.template.ApplicationTe, Заявка; поля соответствуют таблице `applications` и типичным JOIN с `users`., Заявка; поля соответствуют таблице `applications` и типичным JOIN с `users`., Template
...
Cohesion: 0.25
Nodes (7): _normalize_submit_memo_args(), process_free_form_chat(), Free-form диалог для сбора данных заявки., build_submit_memo_model(), LLMError, Модель для OpenAI tool submit_memo: поля совпадают с блоками шаблона., Модель для OpenAI tool submit_memo: поля совпадают с блоками шаблона.
...
Cohesion: 0.47
Nodes (5): _block_title(), on_edit(), Текущий шаг cur идёт позже блока bid в шаблоне (юзер уже продвинулся вперёд)., _remember_cleanup_message(), _tpl_block_order_after()
...
## Knowledge Gaps
- **514 isolated node(s):** `Подписанная cookie админ-сессии (HMAC). Без секрета — только для разработки.`, `Разбирает cookie; при WEB_SESSION_SECRET отвергает подделку.`, `Только удаление строки в форме (без записи в БД).      HTMX по умолчанию не дела`, `Фрагмент HTMX: виджеты счётчиков (обновление без перезагрузки).`, `Возвращает уже согласованную заявку на доработку (только суперпользователь).` (+509 more)
...
- **Why does `get_template()` connect `Community 34` to `Community 1`, `Community 3`, `Community 35`, `Community 39`, `Community 72`, `Community 9`, `Community 11`, `Community 12`, `Community 46`, `Community 14`, `Community 49`, `Community 53`, `Community 22`, `Community 23`, `Community 56`, `Community 61`, `Community 31`?**
  _High betweenness centrality (0.212) - this node is a cross-community bridge._
- **Why does `generate_pdf()` connect `Community 14` to `Community 0`, `Community 34`, `Community 38`, `Community 15`, `Community 19`, `Community 22`?**
  _High betweenness centrality (0.078) - this node is a cross-community bridge._
- **Why does `invalidate_pdf_cache()` connect `Community 19` to `Community 66`, `Community 72`, `Community 74`, `Community 11`, `Community 46`, `Community 15`, `Community 22`, `Community 25`, `Community 59`?**
