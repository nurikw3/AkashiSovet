...
Cohesion: 0.12
Nodes (19): main(), _append_section_paragraphs(), generate_pdf(), _generate_pdf_sync(), _load_signature_for_user(), _normalize_pdf_user_text(), _normalize_risk_placeholder(), _parse_attachments_field() (+11 more)
...
Cohesion: 0.16
Nodes (17): _build_pdf_sync(), _escape_para_text(), _normalize_list_text(), PDF-генерация служебной записки через ReportLab с фоном из шаблона., ReportLab Paragraph — экранирование XML-символов в пользовательском тексте., ReportLab Paragraph — экранирование XML-символов в пользовательском тексте., ReportLab Paragraph — экранирование XML-символов в пользовательском тексте., Нормализует разделители и пробелы, чтобы сплит по «; 2)» срабатывал стабильно. (+9 more)
...
Cohesion: 0.13
Nodes (15): Сохраняет список вложений как в БД (в т.ч. реестр файлов из Telegram до выгрузки, save_all_blocks(), save_attachments_payload(), save_block(), _cache_key(), _get_redis(), invalidate_pdf_cache(), Инвалидирует кэш PDF для заявки после изменения данных.      Сейчас `get_app_pdf (+7 more)

### Community 22 - "Community 22"
Cohesion: 0.14
Nodes (14): meeting_update_schedule(), _parse_meeting_schedule(), Парсит `scheduled_at` (datetime-local) или устаревшее поле даты (10:00)., Парсит `scheduled_at` (datetime-local) или устаревшее поле даты (10:00)., Парсит `scheduled_at` (datetime-local) или устаревшее поле даты (10:00)., Парсит `scheduled_at` (datetime-local) или устаревшее поле даты (10:00)., Парсит `scheduled_at` (datetime-local) или устаревшее поле даты (10:00)., Парсит `scheduled_at` (datetime-local) или устаревшее поле даты (10:00). (+6 more)
...
Cohesion: 0.33
Nodes (6): draw_last_page(), Логотип + футер — только на последней странице., Логотип + футер — только на последней странице., Логотип + футер — только на последней странице., Логотип + футер — только на последней странице., Логотип + футер — только на последней странице.
...
## Knowledge Gaps
- **389 isolated node(s):** `Подписанная cookie админ-сессии (HMAC). Без секрета — только для разработки.`, `Разбирает cookie; при WEB_SESSION_SECRET отвергает подделку.`, `Только удаление строки в форме (без записи в БД).      HTMX по умолчанию не дела`, `Фрагмент HTMX: виджеты счётчиков (обновление без перезагрузки).`, `Скачивание файла по Telegram file_id (вложения, загруженные в боте до S3).` (+384 more)
...
- **Why does `get_template()` connect `Community 3` to `Community 1`, `Community 4`, `Community 9`, `Community 10`, `Community 12`, `Community 18`, `Community 19`, `Community 20`?**
  _High betweenness centrality (0.195) - this node is a cross-community bridge._
- **Why does `generate_pdf()` connect `Community 12` to `Community 33`, `Community 2`, `Community 3`, `Community 14`, `Community 52`, `Community 20`?**
  _High betweenness centrality (0.088) - this node is a cross-community bridge._
- **Why does `invalidate_pdf_cache()` connect `Community 21` to `Community 3`, `Community 10`, `Community 44`, `Community 12`, `Community 14`, `Community 16`, `Community 49`, `Community 19`, `Community 60`?**
